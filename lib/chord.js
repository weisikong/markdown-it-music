...
const CHORD_PATTERN = new RegExp(
  [
    ...
    "△|△7|△9|△11|△13|△9#11|△13#11|△7b5|△7#5|",
    "m|m7|m9|m9/6|m11|m13|m6|madd9|m6add9|m7add9|mmaj7|mmaj9|m7b5|m7#5|",
    "5|",
    "6|6/9|",
    "7|7/6|7sus4|7b5|7#5|ø|7b9|7#9|7b5b9|7b5#9|7#5b9|7b9#5|",
...
function transpose(note, amount, sharpFlatPref) {
  if (note === "" || note === "N.C.") {
    return note;
  }
  ...
  if (sharpFlatPref === 'b') {
    chordSequence = CHORD_SEQUENCE_FLAT;
  } else if (sharpFlatPref === '#') {
    chordSequence = CHORD_SEQUENCE_SHARP;
  }
...
export class Chord {
  ...
  transpose(amount, sharpFlatPref) {
    return new Chord(
      transpose(this.root, amount, sharpFlatPref),
      this.quality,
      transpose(this.bass, amount, sharpFlatPref),
    );
  }
